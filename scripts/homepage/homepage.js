  ...
  static getUserCardDOM(data) {
    ...
    photographers.map((photographer) => {
      ...
      let photographerTemplate = `
              <a href="photographer.html?id=${photographer.id}" title="Lieu ${photographer.city}, ${photographer.country}. Tarif ${photographer.price}euros par jour.">
                  <img src="${picture}" alt="Photo du profil de ${photographer.name}">
                  <h2>${photographer.name}</h2>
              </a>
              <h3>${photographer.city}, ${photographer.country}</h3>
              <h4>${photographer.tagline}</h4>
              <h5>${photographer.price}€/jour</h5>`;
